...
10. [可复制的代码：登录态（Auth.js / next-auth v5）](#10-可复制的代码登录态authjs--next-auth-v5)
...
1. **Cookie 里存 JWT**：服务端签名，浏览器自动带 Cookie，适合 BFF。  
2. **服务端 Session + Cookie 只存 Session ID**：Session 数据在 Redis/数据库，可立即吊销，更安全但运维成本高。  
3. **交给 Auth.js（next-auth v5）统一管理**：由 Auth.js 负责会话 Cookie、回调、保护路由等，你只需要实现「如何验证手机号 + 验证码」这一步（本文第 10 节会给出可复制配置）。

学习阶段可先跑通「验证码校验」与「会话落地」两件事；如果你准备把鉴权做成可扩展（后续加 OAuth、邮箱等），建议直接用 **Auth.js**。
...
sequenceDiagram
  ...
  participant API as Next.js Route Handler / Auth.js
  ...
  alt 校验成功
    API->>API: 查找或创建用户
    API->>API: Auth.js 创建会话并写入 Cookie
    API-->>U: { ok }
  else 失败
    API-->>U: { ok:false } 统一文案
  end
...
src/
  ...
  auth.ts                   # Auth.js（next-auth v5）配置入口（约定）
  app/
    api/
      auth/
        send-code/route.ts
        [...nextauth]/route.ts  # Auth.js route handler（约定）
...
> 如果你要接入 Auth.js（next-auth v5），更推荐把「校验验证码 + 建立会话」放进 Auth.js 的 `Credentials` provider 里（见第 10 节）。  
> 下面这个 `verify-code` route 依然可以保留用于“纯 API 登录”或做内部复用；但使用 Auth.js 时，UI 一般不再直接调用它。
...
export async function POST(req: Request) {
  ...
  // TODO: 查找或创建用户；建立登录态（推荐交给 Auth.js，见下一节）
...
## 10. 可复制的代码：登录态（Auth.js / next-auth v5）

> 目标：把“验证码校验成功后的会话”交给 Auth.js 管理（Cookie / Session / 回调 / 保护路由），你只实现「如何验证 phone + code」。
...
### 10.2 环境变量（与本项目 `.env.example` 对齐）

- **`AUTH_SECRET`**：Auth.js 用于签名/加密（不要使用 `NEXT_PUBLIC_*`）。  
- （可选）**`AUTH_URL`**：某些部署场景需要（例如指定站点 URL）。

### 10.3 新增 `src/auth.ts`（Auth.js 配置入口）

> 核心：用 `Credentials` provider 实现手机号 + 验证码登录。校验通过后返回用户对象，Auth.js 会创建会话 Cookie。
...
### 10.4 新增 `src/app/api/auth/[...nextauth]/route.ts`

> App Router 约定：把 `handlers` 导出为 GET/POST。
...
### 10.5 在服务端读取会话（Server Component / Route Handler / Middleware）
...
6. 登录成功后：Auth.js 会话 Cookie 存在且刷新页面仍保持登录（服务端 `auth()` / 客户端 `useSession()` 均可读到）。
